...
int main(int argc, const char *argv[])
{
    ...
    bool bLog = true;                         // log console outputs in text files
    string detectorType = "";
    ...
    if (argc != 3){
        ...
        cout << "Usage: /.3D_Object_Tracking [detectorType] [dscriptorType] [bLog]" << endl;
        ...
        if (std::find(detectorTypes.begin(), detectorTypes.end(), argv[1]) != detectorTypes.end()){
            detectorType = argv[1];
        }else{
            cout << "Error: Wrong detector name!" << endl;
            cout << "[detectorType]: \tSHITOMASHI, HARRIS, FAST, BRISK, ORB, AKAZE, SIFT" << endl;
            exit(-1);            
        }
        if (std::find(descriptorTypes.begin(), descriptorTypes.end(), argv[2]) != descriptorTypes.end()){
            descriptorType = argv[2];
        }else{
            cout << "Error: Wrong descriptor name!" << endl;
            cout << "[dscriptorType]: \tBRISK, BRIEF, ORB, FREAK, AKAZE, SIFT" << endl;
            exit(-1);
        }
    }
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
    {
        ...
        cout << "\nLoad imag " << imgFullFilename << endl;
        ...
        if(bVis)
        {
            showLidarTopview(lidarPoints, cv::Size(4.0, 20.0), cv::Size(1000, 600), bWait);
            show3DObjects((dataBuffer.end()-1)->boundingBoxes, cv::Size(2.0, 10.0), cv::Size(500, 300), bWait);
        }
        bVis = false;
        ...
        // keep keypoints in the preceeding vehicle
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                {
                    ...
                    if (bVis)
                    {
                        ...
                        putText(visImg, str, cv::Point2f(80, 50), cv::FONT_HERSHEY_PLAIN, 2.5, cv::Scalar(0,0,255));
                        putText(visImg, "Image#"+imgNumber.str(), cv::Point2f(80, 80), cv::FONT_HERSHEY_PLAIN, 2.5, cv::Scalar(0,0,255));

                        string windowName = "Time to collision (TTC)";
